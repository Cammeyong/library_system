...
router.post('/login', function(req, res) {
    ...
    conn.query("SELECT * FROM library_system.admin WHERE email = ? AND BINARY password = ?  [email, password], UNION SELECT * FROM library_system.students WHERE email = ? AND BINARY password = ?" , [email, password], function (err, rows) {
        ...
        if (rows.length <= 0) {
            ...
            // req.session.student_id = rows[0].authorized_user;
